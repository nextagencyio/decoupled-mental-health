...
import { GET_THERAPIST_BY_PATH } from '@/lib/queries'
import { DrupalTherapist } from '@/lib/types'
...
interface TherapistByPathData {
  route: {
    entity: DrupalTherapist
  } | null
}
...
async function getTherapist(path: string): Promise<DrupalTherapist | null> {
  try {
    ...
    const { data } = await apolloClient.query<TherapistByPathData>({
      query: GET_THERAPIST_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getTherapist(path)

  if (!item) {
    return { title: 'Therapist Not Found | Mental Health' }
  }

  return {
    title: `${item.title} | Mental Health`,
                ...
                <dl className="space-y-4">
                  {(item as any).credentials && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).credentials}</dd>
                    </div>
                  )}
                  {(item as any).licenseNumber && (
                    <div>
                      <dt className="text-sm text-gray-500">License Number</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).licenseNumber}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
