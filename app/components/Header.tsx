...
export default function Header() {
  ...
  return (
    <header
      className={clsx('fixed w-full z-50 transition-all duration-300', scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm' : 'bg-white/80 backdrop-blur-md')}
        ...
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="flex items-center space-x-3 hover:opacity-80 transition-opacity duration-200">
            <div className="flex-shrink-0 w-10 h-10 bg-primary-800 rounded-full flex items-center justify-center">
              <Stethoscope className="w-5 h-5 text-accent-300" />
            </div>
            <span className="text-lg font-display font-bold text-primary-900 hidden sm:block">Clearpath Wellness</span>
          </Link>
