...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Service Not Found | Mental Health' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).sessionFormat && (
                    <div>
                      <dt className="text-sm text-gray-500">Session Format</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).sessionFormat}</dd>
                    </div>
                  )}
                  {(item as any).sessionDuration && (
                    <div>
                      <dt className="text-sm text-gray-500">Session Duration</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).sessionDuration}</dd>
                    </div>
                  )}
                  {(item as any).ageGroup && (
                    <div>
