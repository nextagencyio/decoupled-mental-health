...
import { GET_RESOURCE_BY_PATH } from '@/lib/queries'
...
interface ResourceByPathData {
  route: {
    entity: DrupalResource
  } | null
}
...
async function getResource(path: string): Promise<DrupalResource | null> {
  try {
    ...
    const { data } = await apolloClient.query<ResourceByPathData>({
      query: GET_RESOURCE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getResource(path)

  if (!item) {
    return { title: 'Resource Not Found | Mental Health' }
  }
